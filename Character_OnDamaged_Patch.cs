...
// Type: BetterArchery.Character_OnDamaged_Patch
  ...
  public static class Character_OnDamaged_Patch
  {
    public static void Postfix(ref Character __instance, HitData hit)
    {
      if (!hit.HaveAttacker() || !hit.GetAttacker().IsPlayer() || !BetterArchery.ShowSneakDamage.Value || __instance.InAttack() || __instance.m_baseAI.IsAlerted())
        return;
      hit.GetAttacker().Message(MessageHud.MessageType.TopLeft, string.Format("<size=25>Sneak attack for {0}X ({1}) damage!</size>", (object) hit.m_backstabBonus, (object) hit.GetTotalDamage().ToString("0.0")));
    }
  }
}
